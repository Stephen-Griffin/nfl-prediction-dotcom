...
function isDockLabelElement(node: React.ReactNode): node is React.ReactElement<DockLabelProps> {
  return React.isValidElement(node) && node.type === DockLabel;
}
...
function DockItem({
  ...
  return (
    <motion.div
      ...
      aria-haspopup="true"
    >
      {Children.map(children, child => (isDockLabelElement(child) ? cloneElement(child, { isHovered }) : child))}
    </motion.div>
  );
}
  ...
  return (
    <motion.div style={{ height, scrollbarWidth: 'none' }} className="mx-2 flex max-w-full items-center">
      <motion.div
        ...
        className={`${className} fixed z-50 bottom-2 left-1/2 transform -translate-x-1/2 flex items-end w-fit gap-4 rounded-2xl border-neutral-700 border-2 pb-2 px-4`}
